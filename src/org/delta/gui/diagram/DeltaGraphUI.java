...
public class DeltaGraphUI extends BasicGraphUI implements Serializable {
     ...
     * Note: Overridden to fix apparent bug where setting the model after
     * the GraphLayoutCache caused a NullPointerException sometimes.
     */
    @Override
    protected final void prepareForUIInstall() {
     ...
     * @return a CellHandle for the cells in the given context.
     */
    @Override
    public final CellHandle createHandle(final GraphContext context) {
     ...
     * @author Group Delta 2009
     */
    public class DeltaRootHandle extends BasicGraphUI.RootHandle
                                                    implements Serializable {
        /** Needed for correct serialization. */
        private static final long serialVersionUID = 1L;

        /**
         * Creates a new DeltaRootHandle by calling the superclass constructor.
         * @param ctx - graph context with cells to create a root handle for.
         */
        public DeltaRootHandle(final GraphContext ctx) {
            super(ctx);
        }

        /**
         * Process mouse dragged event. Overridden to realign control points
         * during the drag.
                    ...
                    if (isMoving || Math.abs(dx) > thresh
                        ...
                        if (dx != 0 || dy != 0) {
                            ...
                            if (cachedBounds != null) {
                                ...
                                // If any of the views are edges, realign
        ...
        /**
         * Overriden mouseReleased event. Used to realign points on edges
         * attached to a component after it has been moved.
            ...
            try {
                if (event != null && !event.isConsumed()) {
                    if (activeHandle != null) {
                        ...
                        /*
                         * Realign points on edges around the components that
                         * have been moved.
                         */
                        ...
                        int startPoint = 0;
                        // For each edge, find its view and call realignPoints
