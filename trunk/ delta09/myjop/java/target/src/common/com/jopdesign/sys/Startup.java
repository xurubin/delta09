...
public class Startup {
	...
	static void boot() {
		...
		if (Native.rdMem(Const.IO_CPU_ID) == 0)	{
			...
			/*
			Native.wrMem(1, Const.IO_DE2_HEX1);
			Native.wrMem(8, Const.IO_DE2_HEX2);
			//while (data != 0xFF) 
			{
				Native.wrMem(7-Native.rdMem(Const.IO_DE2_HEX1), Const.IO_DE2_HEX1);
				while ((status & Const.MSK_UA_RDRF) == 0) {
					status = Native.rdMem(Const.IO_STATUS);
				}
				data = Native.rdMem(Const.IO_DATA);
				Native.wrMem(data, Const.IO_DE2_LED);
			
				Native.wrMem(7-Native.rdMem(Const.IO_DE2_HEX2), Const.IO_DE2_HEX2);
				key = Native.rdMem(Const.IO_DE2_SWITCH);
				//while (key == Native.rdMem(Const.IO_DE2_SWITCH)){
				//	key = Native.rdMem(Const.IO_DE2_SWITCH);
				//}
			}
			*/
			...
			Native.wr(mem_size, Const.IO_DE2_LED);
			for (val = 0x8000; val <mem_size; val++)
				Native.wr(val, val);
			for (val = 0x8000; val <mem_size; val++)
				if (Native.rd(val) !=val ) {
					Native.wr(val, Const.IO_DE2_LED);
					Native.wr(0xFFFFFF, Const.IO_DE2_HEX1);
					break;
			}
			while (val <=mem_size) {}
			*/
	...
	static void clazzinit() {
		...
		for (int i=0; i<cnt; ++i) {
			...
			if (len<256 && len!=0) {	// see JOPizer constant on max. method length
				//System.out.print("Native invoke ");
				//System.out.println(addr);
				Native.invoke(addr);
